...
from pre_config import get_config

args = get_config()
class HIST_VCM(object):
    root = osp.join(args.datasets_path, 'HITSZ-VCM/')
    # training data
    ...
    def __init__(self, min_seq_len=12):
        ...
        save_path = osp.join(args.datasets_path, 'HITSZ-VCM/')
...
def path_and_save(path_list, dataset_name):
    ...
    for i in tqdm(range(len(path_list)), desc='Path'):
        ...
        for j in random_choice_list:
            ...
            total_images_path.append(path_list[i][0][j])
    ...
    np.save(dataset_name + '_path_path.npy', np.array(total_images_path))
