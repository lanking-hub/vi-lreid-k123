...
class build_vision_transformer(nn.Module):
    ...
    def merge_param(self, trained_path, alpha):
        param_dict = torch.load(trained_path)
        ...
        for i in model_new_state_dict:
            if self.state_dict()[i].shape == model_new_state_dict[i].shape:
                self.state_dict()[i].copy_(model_new_state_dict[i])
            else:
                print (i, " Not Match, Skip!")
